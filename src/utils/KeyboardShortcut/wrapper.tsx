...
const KeyboardWrapper = ({ children, label, combination, onDown }: KeyboardWrapperProps) => {
    const [holded, setHolded] = useState(false);

    let lastKeyUpAt = new Date();

    let keyUpListenerKey: number | undefined, keyDownListenerKey: number | undefined;
    ...
    const _customChildNode = (l: string) => {
        return <><div className='keyboard-shortcut-indicator ' style={{
        }}></div>
